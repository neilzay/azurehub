...
-- SELL_FRUIT_SELECTED: set of labels chosen in the multi-select Sell dropdown.
-- Empty table = sell All. Otherwise only the labelled fruits are sold.
local SELL_FRUIT_SELECTED = {}   -- {} means "All"
    ...
    pcall(function()
        if isfile(ConfigName) then
            ...
            if decoded then
                ...
                if decoded.SELL_FRUIT_SELECTED and type(decoded.SELL_FRUIT_SELECTED) == "table" then
                    SELL_FRUIT_SELECTED = decoded.SELL_FRUIT_SELECTED
                end
            end
        end
    end)
...
-- -------------------------------------------------------
-- Exact DisplayNames from Orchard_ModuleScript Mutations table.
-- Key = mutation key used in the fruit data payload.
-- Value = DisplayName shown in-game.
local MUTATION_KEY_NAMES = {
    -- Value (Cash) mutations
    Value1       = "Tasty",
    Value2       = "Juicy",
    Value3       = "Exotic",
    Value4       = "Perfect",
    -- Rate (Speed) mutations
    Rate1        = "Tart",
    Rate2        = "Zesty",
    Rate3        = "Wild",
    Rate4        = "Blessed",
    -- GrowthRate mutations
    GrowthRate1  = "Fast",
    -- Bad mutations
    GrowthRateBad1 = "Slow",
    ValueBad1    = "Draining",
...
local function buildFruitLabel(entry, index)
    ...
    -- ── Fruit base name: just the evolution name (e.g. "Lemon", "Orange") ──
    local fruitName = nil
    if fruitObj then
        -- Try :GetEvolutionName() or derive from EVOLUTION_NAMES
        pcall(function()
            local n = fruitObj:GetEvolutionName and fruitObj:GetEvolutionName()
            if type(n) == "string" and #n > 0 then fruitName = n end
        end)
    end
    if not fruitName then
        fruitName = (type(evoIndex) == "number" and EVOLUTION_NAMES[evoIndex + 1])
                 or ("Evo#" .. tostring(evoIndex))
    end

    -- ── Build mutation list: "MutName Nx" sorted by priority ─────────────
    -- mutations table: { [mutKey] = count (number) }
    -- Format per request: FruitName (Mut1 Nx, Mut2 Nx)
    local mutParts = {}
    if type(mutations) == "table" then
        for mutKey, val in pairs(mutations) do
            ...
            if count > 0 then
                local displayName = mutationKeyToDisplay(mutKey)
                table.insert(mutParts, { name = displayName, count = count, key = mutKey })
            end
        end
        -- Sort by mutation priority order (from Orchard config), lower Order = higher priority
        -- Priority order matches the game's own display (e.g. Perfect before Tasty)
        local MUTATION_ORDER = {
            Value4=1,Value3=2,Value2=3,Value1=4,
            Rate4=11,Rate3=12,Rate2=13,Rate1=14,
            ValueBad1=5,ValueBad2=6,RateBad1=15,RateBad2=16,
            GrowthRate1=101,GrowthRateBad1=102,
            WalkSpeed1=111,Luck1=21,Void=21,Slimy=22,Giant=23,
        }
        table.sort(mutParts, function(a, b)
            local oa = MUTATION_ORDER[a.key] or 999
            local ob = MUTATION_ORDER[b.key] or 999
    ...
    if #mutParts > 0 then
        local parts = {}
        for _, m in ipairs(mutParts) do
            -- Format: "Perfect 1X" — uppercase X as shown in game UI
            table.insert(parts, m.name .. " " .. m.count .. "X")
        end
        label = fruitName .. " (" .. table.concat(parts, ", ") .. ")"
    else
        label = fruitName
    end
...
local sellDropdownRef = nil  -- WindUI MultiDropdown ref
...
local function rebuildInventoryDropdown()
    ...
    if sellDropdownRef and sellDropdownRef.SetValues then
        pcall(function()
            -- Rebuild multi-select values (no "All" entry — empty selection = sell all)
            local multiVals = {}
            for _, lbl in ipairs(fruitInventoryLabels) do
                if lbl ~= "All" then table.insert(multiVals, lbl) end
            end
            sellDropdownRef:SetValues(multiVals)
            -- Prune any previously-checked labels that no longer exist in inventory
            for label, _ in pairs(SELL_FRUIT_SELECTED) do
                if not fruitInventoryMap[label] then
                    SELL_FRUIT_SELECTED[label] = nil
                end
            end
        end)
    end
...
-- -------------------------------------------------------
-- AUTO SELL FRUIT
-- SELL_FRUIT_SELECTED = {}             → sell all fruits in inventory
-- SELL_FRUIT_SELECTED = {label=true}  → sell only the checked fruits
-- -------------------------------------------------------
...
local function runAutoSellFruit()
    ...
    while true do
        ...
        -- Cobalt confirms: tycoon.Values.SellFruits (NOT tycoon.Remotes)
        local valuesFolder = t:FindFirstChild("Values")
        if not valuesFolder then continue end
        local sellRF = valuesFolder:FindFirstChild("SellFruits")
        if not sellRF then continue end

        local fruitsToSend = {}
        local countsToSend = {}
        local sellAll = next(SELL_FRUIT_SELECTED) == nil  -- empty table = sell all

        if sellAll then
            -- Batch every fruit in inventory (no limit — server will cap if needed)
            local entries = getOrchardFruitEntries()
            for _, entry in ipairs(entries) do
                ...
                table.insert(fruitsToSend, { "_C_", "OrchardFruit", evo, mut })
                table.insert(countsToSend, cnt)
            end
        else
            -- Sell only the fruits the player checked in the multi-select dropdown
            for label, _ in pairs(SELL_FRUIT_SELECTED) do
                local mapped = fruitInventoryMap[label]
                if not mapped then continue end
                -- mapped[1]=evo(0-based), mapped[2]=mutations, mapped[3]=count
                table.insert(fruitsToSend, { "_C_", "OrchardFruit", mapped[1], mapped[2] })
        ...
        sellLock = true
        task.spawn(function()
            -- Cobalt confirms payload: { Fruits = {...}, Counts = {...} }
            local ok = pcall(function()
                sellRF:InvokeServer({
                    Fruits = fruitsToSend,
                    Counts = countsToSend,
...
-- -------------------------------------------------------
-- NO RENDER
-- Only the Baseplate and local character remain visible.
-- Every other workspace object (tycoon, map, players, etc.)
-- is hidden client-side using LocalTransparencyModifier = 1
-- for BaseParts, Transparency = 1 for Decals/Textures, and
-- Enabled = false for particles/effects.
-- DescendantAdded connections on each top-level workspace
-- child block newly-streamed/spawned objects immediately.
-- Lighting is zeroed out. UI (PlayerGui/CoreGui) untouched.
-- -------------------------------------------------------
local noRenderConns   = {}  -- { RBXScriptConnection } per top-level child
local noRenderParts   = {}  -- { part, ltm }
local noRenderVisuals = {}  -- { obj, prop, val }
local noRenderObjects = {}  -- { obj, parent }  (Sky, Atmosphere)
local noRenderLighting = {} -- { prop, val }

-- Returns true for objects we must NEVER hide
local function isAllowed(obj)
    if obj.Name == "Baseplate" then return true end
    if obj:IsA("Terrain") then return true end
    local char = player.Character
    if char and (obj == char or obj:IsDescendantOf(char)) then return true end
    -- Never touch anything inside PlayerGui or CoreGui
...
local function hideObject(obj)
    if isAllowed(obj) then return end

    if obj:IsA("BasePart") then
        -- Client-only; does not replicate. Stacks with server Transparency.
        local prev = obj.LocalTransparencyModifier
        obj.LocalTransparencyModifier = 1
        table.insert(noRenderParts, { part = obj, ltm = prev })

    elseif obj:IsA("Decal") or obj:IsA("Texture") or obj:IsA("SurfaceAppearance") then
        local prev = obj.Transparency
        obj.Transparency = 1
        table.insert(noRenderVisuals, { obj = obj, prop = "Transparency", val = prev })
    ...
    elseif obj:IsA("ParticleEmitter") or obj:IsA("Trail")
        ...
        obj.Enabled = false
        table.insert(noRenderVisuals, { obj = obj, prop = "Enabled", val = prev })
    ...
    elseif obj:IsA("SpecialMesh") or obj:IsA("SelectionBox") then
        -- Hide meshes by making the owning part invisible above
        -- (already handled via BasePart branch). Nothing extra needed.
        return
    end
end

-- Attaches a DescendantAdded blocker to a specific top-level workspace child
-- so newly streamed/spawned objects inside it are hidden immediately.
local function watchFolder(folder)
    local conn = folder.DescendantAdded:Connect(function(obj)
        if noRenderActive then
            task.defer(function()
                if noRenderActive then pcall(hideObject, obj) end
            end)
        end
    end)
    table.insert(noRenderConns, conn)
end
...
local function enableNoRender()
    ...
    -- Save & zero lighting (keeps the engine running but scene goes black)
    local lightingProps = {
        "Brightness", "GlobalShadows", "Ambient", "OutdoorAmbient",
        "FogColor", "FogEnd", "FogStart", "EnvironmentDiffuseScale",
        "EnvironmentSpecularScale",
    }
    noRenderLighting = {}
    for _, prop in ipairs(lightingProps) do
        pcall(function()
            table.insert(noRenderLighting, { prop = prop, val = lighting[prop] })
        end)
    end
    ...
    -- Full-black CCE to catch any emissive or self-lit surfaces
    ...
    -- Also hide all Lighting children except our CCE (Sky, Atmosphere, etc.)
    for _, obj in ipairs(lighting:GetChildren()) do
        if obj ~= noRenderCCE then
            table.insert(noRenderObjects, { obj = obj, parent = lighting })
            obj.Parent = nil
        end
    end
    ...
    noRenderParts   = {}
    noRenderVisuals = {}
    -- noRenderObjects already has Lighting children above; keep them

    -- Hide every current workspace descendant except Baseplate/character
    for _, obj in ipairs(workspace:GetDescendants()) do
        pcall(hideObject, obj)
    end

    -- Watch every top-level workspace child individually so new objects
    -- streaming into any folder (Tycoon1, Tycoon2, Map, Players...) are
    -- caught and hidden immediately without watching all of workspace.
    noRenderConns = {}
    for _, child in ipairs(workspace:GetChildren()) do
        if child.Name ~= "Baseplate" and not child:IsA("Terrain") then
            watchFolder(child)
        end
    end
    -- Also catch brand-new top-level folders being added
    local topConn = workspace.ChildAdded:Connect(function(child)
        if noRenderActive and child.Name ~= "Baseplate" and not child:IsA("Terrain") then
            -- hide any existing descendants immediately
    ...
    table.insert(noRenderConns, topConn)
end
...
local function disableNoRender()
    ...
    -- Disconnect all per-folder watchers
    for _, conn in ipairs(noRenderConns) do
        pcall(function() conn:Disconnect() end)
    end
    noRenderConns = {}

    -- Restore BaseParts
    for _, entry in ipairs(noRenderParts) do
        pcall(function() entry.part.LocalTransparencyModifier = entry.ltm end)
    end
    noRenderParts = {}

    -- Restore visual objects (Decals, Particles, Lights, etc.)
    for _, entry in ipairs(noRenderVisuals) do
        pcall(function() entry.obj[entry.prop] = entry.val end)
    end
    noRenderVisuals = {}

    -- Restore reparented objects (Sky, Atmosphere, Lighting children)
    ...
    local lighting = game:GetService("Lighting")
    for _, entry in ipairs(noRenderLighting) do
        pcall(function() lighting[entry.prop] = entry.val end)
    end
    noRenderLighting = {}
end
...
-- Multi-select sell dropdown.
-- Nothing checked = sell All. One or more checked = sell only those fruits.
-- fruitInventoryLabels[1] is always "All" — excluded from the multi-select
-- since "sell all" is now the default when nothing is ticked.
local sellMultiValues = {}
for i, lbl in ipairs(fruitInventoryLabels) do
    if lbl ~= "All" then
        table.insert(sellMultiValues, lbl)
    end
end

SELL_FRUIT_SELECTED = {}  -- reset to "sell all" on UI build

sellDropdownRef = OrchardTab:MultiDropdown({
    Title    = "Sell: Select Fruits (none = All)",
    Values   = sellMultiValues,
    Default  = {},
    Callback = function(selected)
        -- selected is a table of { [label] = true } for each checked item
        SELL_FRUIT_SELECTED = selected or {}
    end,
})
